...
/// so a batch larger than a single DuckDB vector is streamed across calls.
#[repr(C)]
...
impl VTab for ArrowVTab {
    ...
    fn func(func: &TableFunctionInfo<Self>, output: &mut DataChunkHandle) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // DuckDB's standard vector size; a DataChunk holds at most this many rows.
        ...
        if offset >= total {
            ...
            // Emit the next chunk; a batch wider than one vector is streamed
            // across repeated func() calls instead of asserting it all fits.
            let len = (total - offset).min(VECTOR_SIZE);
            record_batch_to_duckdb_data_chunk(&rb.slice(offset, len), output)?;
            init_info.offset.store(offset + len, Relaxed);
        }
